...
async def is_admin(chat_id: int, user_id: int, client=None) -> bool:
    client = client or app
    if user_id in await db.get_admins(chat_id, client=client):
        return True
    try:
...
async def reload_admins(chat_id: int, client=None) -> list[int]:
    client = client or app
    try:
        admins = [
            admin
